...
example_product_simplified_schemas_dir: Path = Path(f"{example_product_schemas_dir}/simplified_for_testing")

def load_example_products():
    raise NotImplementedError("Loading example products not supported")
    
def load_example_products_simplified(schema_dir: Path = example_product_simplified_schemas_dir):
    ...
    if not isinstance(schema_dir, Path):
        if isinstance(schema_dir, str):
            ...
            raise TypeError(f"Invalid type for schema_dir: ({type(schema_dir)}). Must be of type Path")
    assert schema_dir.exists(), FileNotFoundError(f"Could not find schema dir: {schema_dir}")
